...
export default function FSMFormSave(clientAPI) {
    ...
    return FSMSmartFormsLibrary.saveCurrentPageValues(context).then(() => {
        ...
        if (!ValidationLibrary.evalIsEmpty(valuesFolderPath)) {
            const formNameValueMap = new Map(); // create a map of key value pair
            ...
            if (formNameValueMap.size || isSubmit) { //Either save is required because of change, or user is submitting form
                ...
                return values_file.readText().then(res => {
                    ...
                    for (const control of formNameValueMap.keys()) { // will only have values that needs update
                        ...
                        const index = res.indexOf(controlName);
                        const indexOfValues = res.indexOf('</values>');
                        if (index !== -1 && formControls[control].Type !== 'attachmentPicker') { // Make sure we only add when the index is found
                            ...
                            const endElementIndex = res.indexOf('>', index);
                            const startElementIndex = res.indexOf('<', index);
                    ...
                    return values_file.writeText(res).then(function() {
                        ...
                        return valuesZipFile.remove().then(() => {
                            ...
                            if (binarySource) {
                                ...
                                return clientAPI.executeAction('/SAPAssetManager/Actions/Forms/FSM/UpdateFormInstance.action').then((result) => {
                                    const isCompletionFlow = IsCompleteAction(clientAPI);
                                    if (isCompletionFlow || SmartFormsConfirmationLibrary.isConfirmationAction(clientAPI)) {
                                        let links = WorkOrderCompletionLibrary.getStep(clientAPI, 'smartforms').links || [];
                                        links.push(JSON.parse(result.data)['@odata.editLink']);
                                        WorkOrderCompletionLibrary.updateStepState(clientAPI, 'smartforms', {
                                            links: links,
                                        });
                                    }
    ...
    function CancelSaveCleanup(sigMap) {
        ...
        for (const control in sigMap) {
            if (FSMSmartFormsLibrary.ifMediaExist(sigMap[control])) { //Remove local signature image
                promises.push(FSMSmartFormsLibrary.deleteLocalMedia(sigMap[control]));
            }
